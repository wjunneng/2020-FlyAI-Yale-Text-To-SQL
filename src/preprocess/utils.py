...
def generate_json():
    ...
    dev_json_data = json.load(fp=open('../../data_yan/input/dev.json'))
    train_others_data = json.load(fp=open('../../data_yan/input/train_others.json'))
    train_spider_data = json.load(fp=open('../../data_yan/input/train_spider.json'))

    from itertools import chain

    irnet = list(chain(*[dev_json_data, train_others_data, train_spider_data]))
    irnet_question = [str(i['question']).lower() for i in irnet]
    irnet = dict(zip(irnet_question, irnet))

    test_csv_data = pd.read_csv('../../data_yan/input/test.csv')
    train_csv_data = pd.read_csv('../../data_yan/input/train.csv')

    count = 0
    test_json_data = []
    for index in range(test_csv_data.shape[0]):
        try:
            sample = irnet[test_csv_data.iloc[index, 1]]
            sample['question'] = sample['question'].lower()
            sample['db_id'] = train_csv_data.iloc[index, 0]
            test_json_data.append(sample)
        except:
            count += 1
            print(test_csv_data.iloc[index, 1])
    ...
    for index in range(train_csv_data.shape[0]):
        try:
            sample = irnet[train_csv_data.iloc[index, 1]]
            sample['question'] = sample['question'].lower()
            sample['db_id'] = train_csv_data.iloc[index, 0]
            train_json_data.append(sample)
        except:
            count += 1
            print(train_csv_data.iloc[index, 1])

    print('count: {}'.format(count))
    with open('../../data_yan/test.json', 'w') as file:
        json.dump(obj=test_json_data, fp=file)
