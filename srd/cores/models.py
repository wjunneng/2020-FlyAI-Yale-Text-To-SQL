...
from srd.libs.utils_wikisql import *
...
class Seq2SQL_v1(nn.Module):
    ...
    def beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, engine, tb,
        ...
        idxs = SqlovaUtil.topk_multi_dim(torch.tensor(prob_sca), n_topk=beam_size, batch_exist=True)
        ...
        idxs = SqlovaUtil.topk_multi_dim(torch.tensor(prob_w), n_topk=beam_size, batch_exist=True)
...
class FT_Scalar_1(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_h, l_hs, cls_vec, g_sc=None, g_sa=None, g_wn=None, g_wc=None, g_wo=None,
                g_wvi=None, show_p_sc=False, show_p_sa=False, show_p_wn=False, show_p_wc=False, show_p_wo=False,
                show_p_wv=False):
    ...
    def forward_EG(self, wemb_n, l_n, wemb_h, l_hs, cls_vec, engine, tb, nlu_t, nlu_tt, tt_to_t_idx, nlu, beam_size=4):
        ...
        """
        # Select-clause
        prob_sca, pr_sc_best, pr_sa_best, p_sc_best, p_sa_best, p_select = self.EG_decoding_select(wemb_h, l_hs, tb,
                                                                                                   beam_size=beam_size)

        # Where-clause
        prob_w, prob_wn_w, pr_wn_based_on_prob, pr_sql_i, pr_wvi_best, p_where, p_wn_best, p_wc_best, p_wo_best, \
        p_wvi_best = self.EG_decoding_where(wemb_n, l_n, wemb_h, l_hs, cls_vec, engine, tb, nlu_t, nlu_tt, tt_to_t_idx,
                                            nlu, pr_sc_best, pr_sa_best, beam_size=4)

        p_tot = cal_prob_tot(p_select, p_where)

        return pr_sc_best, pr_sa_best, pr_wn_based_on_prob, pr_wvi_best, pr_sql_i, p_tot, p_select, p_where, p_sc_best, \
               p_sa_best, p_wn_best, p_wc_best, p_wo_best, p_wvi_best
    ...
    def EG_decoding_where(self, wemb_n, l_n, wemb_h, l_hs, cls_vec, engine, tb, nlu_t, nlu_wp_t, tt_to_t_idx, nlu,
                          pr_sc_best, pr_sa_best, beam_size=4, show_p_wn=False, show_p_wc=False, show_p_wo=False,
                          show_p_wv=False):
